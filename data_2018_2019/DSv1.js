...
var minVal = 1340;       // Min. input value [700us, 2300us] 
...
var repeat = 0;          // Number of times to repeat the same sequence
var filePrefix = "DSv1_2bat_MiniKDE";
...
function startSteps(){
    ...
    rcb.output.steps("escA", minVal, maxVal, stepsQty, stepFct);
}
        ...
        rcb.files.newLogEntry(result,function(){
            if(isLastStep){ // If it is the last step
                if (goingUp && stepsGoDown){
                    ...
                    rcb.console.setVerbose(true);
                    rcb.output.steps("escA", maxVal, minVal, stepsQty, stepFct);
                }else{
                    if(repeat>1){
                        repeat --;
