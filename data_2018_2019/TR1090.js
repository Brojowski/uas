...
var testMin = 1050; // 0% throttle value (us)
var testMax = 2000; // 100% throttle value (us)
...
steps = [10, 90, 10, 90, 10, 90, 10, 90, 10, 90, 10, 90, 10, 90, 10, 90, 10, 90, 10, 90, 10];

// CSV File
var filePrefix = "MEGA_305x97_5b_1090_RBoff_set_run#";
var saveAll = false; // if true, all data points will be recorded. If false, only final point at end of step will be saved.
var extraTime = 1; // adds extra time (seconds) before saving the result and going to the next step after the RPM is stable. This gives a change for the load cells to also stabilize
...
// start new log file
var add = [settlePercent*100 + "% settling time (s)", "Max acceleration (RPM/s)"];
rcb.files.newLogFile({prefix: filePrefix, additionalHeaders: add});

// hide console debug info
rcb.console.setVerbose(false);

rcb.console.warning("It is highly recommended to use the optical probe to obtain accurate acceleration data.");
rcb.console.warning("Plots slow down app which reduces sampling rate. For best accuracy, uncheck all plots.");

// ESC initialization
rcb.console.print("Initializing ESC...");
rcb.output.pwm("escA",minVal);
...
function readDone(result){
    ...
    if(settling){
        ...
        if(stepIndex < steps.length){
            rcb.console.print("Step " + (stepIndex+1) + " of " + steps.length + "...");
            ...
            // set the output
            rcb.output.pwm("escA",Math.round(throttle));
...
function isStable(readings){
    if(waitUntil){
        ...
        //(ie, at least 25% of readings are going up, and at least 25% are going down)
        //and values are within 75 rpm and at least 50 rpm
        var criteria = 0.10*readings.length;
...
function getRPM(result){
    var RPM = 0;
    // Try getting the optical RPM first
    if(result.motorOpticalSpeed){
        RPM = result.motorOpticalSpeed.workingValue;
    }
    if(RPM === 0 && result.motorElectricalSpeed){
        RPM = result.motorElectricalSpeed.workingValue;
    }
    return RPM;
